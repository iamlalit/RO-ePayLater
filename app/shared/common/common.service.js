(function(){
	...
	commonService.$inject = ['$timeout', '$http', '$q', 'enumApp', '$rootScope', '$window'];

	function commonService($timeout, $http, $q, enumApp, $rootScope, $window){
	    ...
	    this.getDashboardData = getDashboardData;
			this.getDashboardOffice = getDashboardOffice;
	    this.dateParser = dateParser;
	    this.callPopover = callPopover;
	    this.printPage = printPage;
			this.setSelectValue = setSelectValue;

	    //////////////////////////////////////////////

	    function init(elemForSelect){
	    	this.sendItems = {
					...
					'officesList' : this.sendItems.officesList != null && this.sendItems.officesList.length > 0 ? this.sendItems.officesList : [],
				};
				this.callPopover();
			  this.setSelectValue(this.sendItems.officesList, elemForSelect);
	    }
	    ...
	    function getDashboardData(){
	      ...
	      $http({
	        ...
	        method : "GET",
	        params : {
	        	'officesList' : checkForEmptyOrNull(nullifyZeroValue(this.sendItems.officesList)),
	      ...
	      return deffered.promise;
			...
			function getDashboardOffice(){
				var deffered = $q.defer();

	      $http({
	        url : enumApp.URL + "GetOccDashBoardOffices",
	        method : "GET",
	        params : {
	        }
	      })
		...
		function nullifyZeroValue(val){
			if(val != null && val.length == 1 && checkIfEqual(val, ['0']))
				return null;
			else
				return val;
		}
		...
		function setSelectValue(value, elemForSelect){
			$timeout(function() {
				$(elemForSelect).val(value).trigger('change');
			}, 500);
		}
